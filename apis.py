...
# ─────────────────────────────────────────────────────────────────────────────
# 13. MANDI PRICES — Agmarknet (FREE Government API)
# ─────────────────────────────────────────────────────────────────────────────
def get_mandi_prices(crop: str = "wheat", state: str = "Uttar Pradesh") -> dict:
    """Get mandi rates from government data / fallback search."""
    try:
        # Try data.gov.in API (free)
        url = "https://api.data.gov.in/resource/9ef84268-d588-465a-a308-a864a43d0070"
        params = {
            "api-key": "579b464db66ec23bdd000001cdd3946e44ce4aad38d4f2b7ac06c5a",
            "format": "json",
        ...
        if data.get("records"):
            ...
            return {"crop": crop, "state": state, "records": records, "source": "Agmarknet"}
    except Exception:
        pass

    # Fallback search
    results = search_duckduckgo(f"{crop} mandi rate today {state} MSP")
    return {"crop": crop, "state": state, "search_results": results, "source": "Web Search"}


# ─────────────────────────────────────────────────────────────────────────────
# 14. CROP DISEASE DETECTION — Prompt for Llama 4 Scout
# ─────────────────────────────────────────────────────────────────────────────
CROP_DISEASE_PROMPT = """You are an expert agricultural scientist with deep knowledge of:
- Crop diseases, pests, and deficiencies
- Indian farming practices (Kharif, Rabi, Zaid seasons)
- All major crops: wheat, rice, sugarcane, cotton, maize, pulses, vegetables
- Organic and chemical treatments available in India
- Pesticides and medicines available at Kisan Seva Kendras

When analyzing a crop/plant image:
1. Identify the crop/plant
2. Diagnose any disease, pest attack, or nutrient deficiency
3. Severity assessment (mild/moderate/severe)
4. Recommended treatment (medicine name, dosage, cost estimate)
5. Prevention tips
6. When to consult an agricultural officer
